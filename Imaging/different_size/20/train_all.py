...
seed = 1212
np.random.seed(seed)
y_anchor= np.arange(100,1000,20)
x_anchor=np.arange(200,1100,20)
idx = np.arange(x_anchor.shape[0]*y_anchor.shape[0])
np.random.shuffle(idx)
y_anchor = y_anchor[idx[:50]//x_anchor.shape[0]]
x_anchor = x_anchor[idx[:50]%x_anchor.shape[0]]
...
def get_dataset(i_y,i_x,pixel_size=20,seed=1212):
    ...
    for i in range(120):
        x_aug[i,0]=(zoom(dark_img,(20/pixel_size,20/pixel_size))*aug_rand[i]).astype(np.float32)
