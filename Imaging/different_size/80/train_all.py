...
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
norm_factor=65536
y_anchor=[310,390,470,550,630,710,790,870]
x_anchor=[310,390,470,550,630,710,790,870]
...
def get_dataset(i_y,i_x,pixel_size=80,seed=1212):
    ...
    x_train=np.zeros((1000*10,1,80,80),dtype=np.float32)
    ...
    x_val=np.zeros((100*10,1,80,80),dtype=np.float32)
    ...
    x_test=np.zeros((100*10,1,80,80),dtype=np.float32)
    ...
    x_aug=np.zeros((120,1,80,80),dtype=np.float32)
    for i in range(120):
        x_aug[i,0]=(zoom(dark_img,(80/pixel_size,80/pixel_size))*aug_rand[i]).astype(np.float32)
