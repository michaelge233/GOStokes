...
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
norm_factor=65536
y_anchor=[70,230,390,550,710,870]
x_anchor=[150,310,470,630,790,950,1110]
...
def train(model,X,y,optimizer,loss_fn,batchsize=64):
...
def evaluate(model,X,y,loss_fn,batchsize=64,calc_MAE=False):
    epoch_loss = 0
...
def run_cnn_exp(task_name,x_train,x_val,x_test,y_train,y_val,y_test,
                p=0.001,seed_list=[324,716,10086],n_epoch=5000,save_dir=save_dir):
    score=[]
...
def get_dataset(i_y,i_x,pixel_size=80,seed=1212):
    ...
    x_train=np.zeros((1000*10,1,160,160),dtype=np.float32)
    ...
    x_val=np.zeros((100*10,1,160,160),dtype=np.float32)
    ...
    x_test=np.zeros((100*10,1,160,160),dtype=np.float32)
    ...
    x_aug=np.zeros((120,1,160,160),dtype=np.float32)
    for i in range(120):
        x_aug[i,0]=(zoom(dark_img,(160/pixel_size,160/pixel_size))*aug_rand[i]).astype(np.float32)
...
for i in range(len(y_anchor)):
    for j in range(len(x_anchor)):
        train_pixel(i,j,pixel_size=160)
print("All finished...")
